...
static bool luax_checkarray(lua_State* L, int index, int start, int count, const DataField* array, char* data) {
  ...
  if (array->fieldCount > 0) {
    ...
    if (type == LUA_TNUMBER) {
      ...
      for (int i = 0; i < count; i++, data += array->stride) {
        lua_rawgeti(L, index, start + i);
        if (!luax_checkfieldm(L, -1, array, data, NULL)) {
          return false;
        }
        lua_pop(L, 1);
      }
      ...
      for (int i = 0; i < count; i++, data += array->stride) {
        lua_rawgeti(L, index, start + i);
        float* matrix = luax_tomat4(L, -1);
        bool success = matrix
          ? luax_checkfieldm(L, -1, array, data, matrix)
...
bool luax_checkbufferdata(lua_State* L, int index, const DataField* field, char* data) {
  ...
  if (field->length > 0) {
    ...
    return luax_checkfieldm(L, index, field, data, NULL);
  } else if (type == LUA_TCDATA
#ifdef LOVR_USE_LUAU
    || type == LUA_TVECTOR
#endif
    ...
    return matrix
      ? luax_checkfieldm(L, index, field, data, matrix)
